...
func TestPrescriptionRoutes(t *testing.T) {
	...
	t.Run("Create Prescription with Medical Record", func(t *testing.T) {
		...
		prescription := factories.SeedPrescription(db, &record.ID, userDoctor.ID, userPatient.ID)

		assert.NotNil(t, prescription.ID)
		assert.Equal(t, userPatient.ID, prescription.PatientID)
		assert.Equal(t, userDoctor.ID, prescription.DoctorID)
		assert.NotNil(t, prescription.MedicalRecordID)
	})

	t.Run("Create Prescription without Medical Record", func(t *testing.T) {
		prescription := factories.SeedPrescription(db, nil, userDoctor.ID, userPatient.ID)
	...
	t.Run("Get Prescription by ID", func(t *testing.T) {
		...
		assert.Contains(t, res.Body.String(), prescription.Medication)
	})

	t.Run("Get Prescriptions by Patient ID", func(t *testing.T) {
		factories.SeedPrescription(db, nil, userDoctor.ID, userPatient.ID)

		res := clientPatient.Get("/prescriptions/patient/"+userPatient.ID.String(), nil)
		assert.Equal(t, http.StatusOK, res.Code)
		assert.Contains(t, res.Body.String(), "TestMed")
	})
